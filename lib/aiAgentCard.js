...
export async function getAIAgentCard({
  ...
  return buildAIAgentSVG({ data, text_color, title_color, bg_color, font_family, chart_color });
}
...
function generatePalette(count) {
  ...
  // Shuffle so similar colors aren't adjacent
...
export function buildAIAgentSVG({ data, text_color = '5f574f', title_color = 'ffff', bg_color = 'f8f6f3', font_family = 'Calibri', chart_color = '9c8f80' }) {
  text_color = text_color.replace(/^#/, '');
  title_color = title_color.replace(/^#/, '');
  bg_color = bg_color.replace(/^#/, '');
  chart_color = chart_color.replace(/^#/, '');
  ...
  // Unique color for every AI agent
  const palette = generatePalette(agents.length);

  const donutBg = isDarkChart
    ? shiftLightness(chart_color, 40)
    : shiftLightness(chart_color, -40);
