...
export function svgContainer({
  ...
  function hexToSvgFill(hex) {
    if (hex.length === 8) {
      const alpha = parseInt(hex.slice(6, 8), 16);
      if (alpha === 0) return { fill: 'none' };
      return { fill: `#${hex.slice(0, 6)}`, 'fill-opacity': (alpha / 255).toFixed(2) };
    }
    return { fill: `#${hex}` };
  }

  function hexToSvgStroke(hex) {
    if (hex.length === 8) {
      const alpha = parseInt(hex.slice(6, 8), 16);
      if (alpha === 0) return { stroke: 'none' };
      return { stroke: `#${hex.slice(0, 6)}`, 'stroke-opacity': (alpha / 255).toFixed(2) };
    }
    return hex ? { stroke: `#${hex}` } : { stroke: 'none' };
  }

  const bgFill = hexToSvgFill(bg_color);
  const borderStroke = hexToSvgStroke(border_width > 0 ? border_color : '');
...
<svg width="${svgEffectiveWidth}" height="${totalHeight}" xmlns="http://www.w3.org/2000/svg" style="font-family:${font_family},sans-serif;">
  <rect
    ...
    ${Object.entries(bgFill).map(([k, v]) => `${k}="${v}"`).join(' ')}
    ${Object.entries(borderStroke).map(([k, v]) => `${k}="${v}"`).join(' ')}
