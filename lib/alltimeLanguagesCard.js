...
export async function getAlltimeLanguagesCard({
    api_key,
    username,
    text_color,
    font_family,
  ...
  const numLangs = parseInt(num_langs, 10) || 5;
    ...
    switch (chart_type) {
      case 'bar': {
        const barBlocks = topLanguages.map((lang, i) => {
            ...
            if (!hide_time) {
            blocks.push(
                `<text font-family="${font_family}" x="${x + barWidth / 2}" y="${y - 4}" font-size="9" text-anchor="middle" fill="#${text_color}">${shortTime}</text>`
            );
            }

            blocks.push(
            `<text font-family="${font_family}" x="${x + barWidth / 2}" y="${chartBase + 12}" font-weight="bold" font-size="10" text-anchor="middle" fill="#${text_color}">${truncateLabel(lang.name, 10)}</text>`
            );

            if (!hide_percentage) {
            blocks.push(
                `<text font-family="${font_family}" x="${x + barWidth / 2}" y="${chartBase + 24}" font-size="9" text-anchor="middle" fill="#${text_color}">${pct}%</text>`
            );
            }
      ...
      case 'bar_vertical': {
        ...
        const barBlocks = topLanguages.map((lang, i) => {
          ...
          blocks.push(`<text font-family="${font_family}" x="${75 + leftPadding - 8}" y="${y + 9}" font-size="10" text-anchor="end" fill="#${text_color}">${truncateLabel(lang.name, 10)}</text>`);
          blocks.push(`<rect x="${75 + leftPadding}" y="${y}" width="${barWidth}" height="12" fill="#${fill}" rx="2" ry="2" />`);

          if (!hide_time && hide_percentage) {
            blocks.push(`<text font-family="${font_family}" x="${75 + leftPadding + barWidth + 6}" y="${y + 9}" font-size="9" fill="#${text_color}">${shortTime}</text>`);
          }

          if (!hide_time && !hide_percentage) {
            blocks.push(`<text font-family="${font_family}" x="${75 + leftPadding + barWidth + 6}" y="${y + 9}" font-size="9" fill="#${text_color}">${shortTime}    |    ${pct}%</text>`);
          }

          if (!hide_percentage && hide_time) {
            blocks.push(`<text font-family="${font_family}" x="${75 + leftPadding + barWidth + 6}" y="${y + 9}" font-size="9" fill="#${text_color}">${pct}%</text>`);
          }
      ...
      case 'radar': {
        ...
        const labels = topLanguages.map((lang, i) => {
          ...
          return `<text font-family="${font_family}" x="${x}" y="${y}" font-size="10" text-anchor="middle" alignment-baseline="middle" fill="#${text_color}">${truncateLabel(lang.name, 6)}</text>`;
        });
        ...
        leaves.forEach((node, i) => {
          ...
          if (!hide_legend) {
            ...
            legendItems.push(`
              ...
              <text font-family="${font_family}" x="${x + legendTextOffset + 70}" y="${y + 3}" font-size="10" fill="#${text_color}">
                ${shortTime} (${pct}%)
              </text>
            `);
          }
        ...
        topLanguages.forEach((lang, i) => {
            ...
            if (!hideOuterLabel) {
                ...
                <text font-family="${font_family}" font-size="9" fill="#${text_color}">
                    <textPath href="#${dayPathId}" font-family="${font_family}" startOffset="${startOffset}" text-anchor="${anchor}">${label}</textPath>
                </text>
                `);
            }
